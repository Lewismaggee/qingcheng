...
public class ReturnOrderServiceImpl implements ReturnOrderService {
    ...
    public void agreeRefund(String id, Integer money, Integer adminId) {
        //1.修改tb_order表pay_status表状态为: 2-已退款


        //2.新增t_return_order表 记录: type=2 status:1
        ...
        orderLog.setPayStatus("2");
        orderLog.setOrderId(returnOrder.getOrderId());
        orderLog.setRemarks(returnOrder.getRemark());
    ...
    public void rejectRefund(String id, String remark, Integer adminId) {
        ...
        //修改return_order表属性
        returnOrder.setStatus("2"); //2:驳回
        returnOrder.setDisposeTime(new Date()); //处理时间
        returnOrder.setAdminId(adminId); //管理员
        returnOrder.setRemark(remark); //理由
        returnOrderMapper.updateByPrimaryKeySelective(returnOrder);

        //修改对应订单明细状态
        Example example = new Example(ReturnOrderItem.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("returnOrderId",returnOrder.getId());
        ...
        for(ReturnOrderItem returnOrderItem : returnOrderItems){
            //根据returnOrderItem中的orderItemId查询orderItem
            Long orderItemId = returnOrderItem.getOrderItemId();
            OrderItem orderItem = new OrderItem();
            orderItem.setId(orderItemId+"");
            orderItem.setIsReturn("0"); //未退款
            orderItemMapper.updateByPrimaryKeySelective(orderItem);//更新订单详情表中
        }
    ...
    @Override
    @Transactional
    public void agreeReSale(String id) {
        //1.根据id查询该服务单是否存在
        ReturnOrder returnOrder = returnOrderMapper.selectByPrimaryKey(id);
        if(returnOrder == null){
     ...
     */
    @Override
    public void rejectResale(String id) {
        //1.根据id查询服务单
        ReturnOrder returnOrder = returnOrderMapper.selectByPrimaryKey(id);
        if(returnOrder == null){
            throw new RuntimeException("退货订单不存在");
