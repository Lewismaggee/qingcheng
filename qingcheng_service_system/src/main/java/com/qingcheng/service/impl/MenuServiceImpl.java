...
public class MenuServiceImpl implements MenuService {
    ...
    public List<Map> findAllMenu() {
        //方式1: 首先按照条件查询上级菜单id为0的列表(1级菜单), 循环得到每个一级菜单的id ,查询二级菜单,嵌套循环得到每个二级菜单id,再查询三级菜单....
        //不推荐使用方式1,因为需要和数据库频繁交互,交互次数=1+一级菜单数量+二级菜单数量
     ...
     * @return
     */
    public List<Menu> findMenuByAuthentication(String loginName) {

        return menuMapper.findMenuByAuthentication(loginName);
    }

    public List<Map> findMenusByAuthentication(String loginName){
        return findMenuListByParentId(findMenuByAuthentication(loginName),"0");
    }
