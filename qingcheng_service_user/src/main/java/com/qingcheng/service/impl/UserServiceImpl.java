...
public class UserServiceImpl implements UserService {
    ...
    public void add(User user, String smsCode) {
        //1.校验
        //1.1 验证码
        String sysCode = (String) redisTemplate.boundValueOps("code_" + user.getPhone()).get(); //提取redis中
        if(sysCode == null){ //redis中验证码不存在
            throw new RuntimeException("验证码不存在或已过期");
        }

        if(!sysCode.equals(smsCode)){ //redis中验证码和输入验证码不相等
