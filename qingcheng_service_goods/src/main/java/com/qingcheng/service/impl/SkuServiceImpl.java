...
public class SkuServiceImpl implements SkuService {
    ...
    public Integer findPrice(String id) {
        Integer skuPrice = (Integer) redisTemplate.boundHashOps(CacheKey.SKU_PRICE).get(id);
        return skuPrice;
    }
    ...
    public boolean deductionStock(List<OrderItem> orderItemList) {
        ...
        for(OrderItem orderItem : orderItemList){
            ...
            if(sku == null){ //数据库中没有该商品
                isDeduction = false;
                break;
            }
            if(!"1".equals(sku.getStatus())){ //商品状态不是正常状态
                isDeduction = false;
                break;
            }
            if(sku.getNum().intValue() < orderItem.getNum().intValue()){ //库存数量不足
                isDeduction = false;
                break;
            }
        }
    ...
    public static void main(String[] args) {
        Integer price = new SkuServiceImpl().findPrice("10794939361");
